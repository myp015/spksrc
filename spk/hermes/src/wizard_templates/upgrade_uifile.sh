...
normalize_workspace() {
  case "$1" in
    */.hermes) printf '%s' "${1%/.hermes}" ;;
    *) printf '%s' "$1" ;;
  esac
}

workspace_home="$(read_first_existing \
  /var/packages/hermes/var/workspace.home.path \
  /var/packages/hermes/var/workspace.home.path)"

if [ -z "$workspace_home" ]; then
  workspace_home="$(read_first_existing \
    /var/packages/hermes/var/workspace.path \
    /var/packages/hermes/var/workspace.path)"
fi

if [ -z "$workspace_home" ]; then
  workspace_home="/volume1/hermes"
fi
workspace_home="$(normalize_workspace "$workspace_home")"

config_path=""
for p in \
  "$workspace_home/.hermes/hermes.json" \
  /var/packages/hermes/var/hermes.json \
  /var/packages/hermes/var/hermes.json \
  "$workspace_home/hermes.json"
do
  if [ -f "$p" ]; then
    config_path="$p"
    break
