#!/bin/sh
APP_VAR_DIR="/var/packages/hermes/var"
if [ -d "/volume1/@appdata/hermes" ]; then
    APP_VAR_DIR="/volume1/@appdata/hermes"
fi
...
# workspace=/volume1/hermes -> cfg=/volume1/hermes/.hermes/hermes.json
CFG_FILE="$(python3 - <<'PY' "$BASE_CFG_FILE"
import json, os, sys
base = sys.argv[1] if len(sys.argv) > 1 else '/volume1/hermes/.hermes/hermes.json'
workspace = '/volume1/hermes'
...
if (not ptr_hit) and workspace == '/volume1/hermes':
    try:
        ...
        try:
            ...
            for line in txt.splitlines():
                for part in line.split():
                    if '/hermes' in part or '/node' in part or 'users:(("node",pid=' in part or 'LISTEN' in line:
                        pid = None
                        if 'pid=' in part:
...
for p in ('/var/packages/hermes/INFO', '/var/packages/hermes/INFO'):
    try:
        ...
        pass
app_ver = ''
for p in ('/var/packages/hermes/target/app/hermes/package.json', '/var/packages/hermes/target/app/hermes/package.json'):
    try:
        if os.path.exists(p):
...
PY
        ...
        models_save)
            ...
            cfg_file="${CFG_FILE}"
            printf 'Content-Type: application/json; charset=UTF-8\r\n\r\n'
            python3 - <<'PY' "$body" "$cfg_file" "${APP_VAR_DIR}/hermes-gateway.spawn.log"
import json, os, sys
raw = sys.argv[1] if len(sys.argv) > 1 else '{}'
cfg_path = sys.argv[2] if len(sys.argv) > 2 else ''
spawn_log = sys.argv[3] if len(sys.argv) > 3 else '/tmp/hermes-gateway.spawn.log'
try:
...
state_path = os.path.join(workspace, '.hermes')
...
if workspace_explicit and workspace_changed:
    try:
        import shutil
        old_state = os.path.join(prev_workspace or '/volume1/hermes', '.hermes')
        new_state = os.path.join(workspace or '/volume1/hermes', '.hermes')
...
if (not os.path.exists(cfg_path)) and os.path.exists('/var/packages/hermes/target/app/hermes/config/hermes.template.json'):
    try:
        cfg = json.load(open('/var/packages/hermes/target/app/hermes/config/hermes.template.json', 'r', encoding='utf-8'))
        qmd = cfg.setdefault('memory', {}).setdefault('qmd', {})
        paths = qmd.setdefault('paths', [])
        state_path = os.path.join(workspace or '/volume1/hermes', '.hermes')
        cfg.setdefault('agents', {}).setdefault('defaults', {})['workspace'] = state_path
        if not paths:
            paths.append({'path': state_path, 'name': 'workspace', 'pattern': '**/*.md'})
        elif isinstance(paths[0], dict):
...
try:
    ...
    if providers_map:
        ...
        # NOTE: service-setup initializes HERMES_CONFIG_FILE to base defaults when sourced.
        # Re-assign target cfg path AFTER source so sync runs on the active workspace config.
        sync_cmd = (
            'bash -lc "source /var/packages/hermes/scripts/service-setup >/dev/null 2>&1; '
            'HERMES_CONFIG_FILE=\"{cfg}\"; '
            'HERMES_CONFIG_PATH=\"{cfg}\"; '
            'HERMES_STATE_DIR=\"{state}\"; '
            'HERMES_WORKSPACE_DIR=\"{workspace}\"; '
            'HOME=\"{workspace}\"; '
            'HERMES_GATEWAY_RESTART_START=1; '
...
try:
    default_state = '/volume1/hermes/.hermes'
    if (workspace or '/volume1/hermes') != '/volume1/hermes':
        import shutil
...
state_dir = os.path.dirname(cfg_path) if cfg_path else os.path.join(workspace or '/volume1/hermes', '.hermes')
skills_dir = os.path.join(state_dir, 'skills', '_bundled')
ext_dir = os.path.join(state_dir, 'extensions')
os.makedirs(skills_dir, exist_ok=True)
os.makedirs(ext_dir, exist_ok=True)

app_dir = '/var/packages/hermes/target/app/hermes'
...
for pkg_name in ['feishu', 'feishu-hermes-plugin', 'dingtalk', 'wecom', 'qqbot', 'hermes-qqbot', 'hermes-weixin']:
    try:
...
if workspace_explicit and workspace_changed:
    try:
        ...
        init_cmd = (
            ...
            'export SYNOPKG_PKGDEST=/var/packages/hermes/target; '
            'export SYNOPKG_PKGVAR=/var/packages/hermes/var; '
            'source /var/packages/hermes/scripts/service-setup >/dev/null 2>&1; '
            'HERMES_CONFIG_FILE=\"{cfg}\"; '
            'HERMES_CONFIG_PATH=\"{cfg}\"; '
            'HERMES_STATE_DIR=\"{state}\"; '
...
hptr_val = ''
try:
    ptr_val = (open('/var/packages/hermes/var/workspace.path', 'r', encoding='utf-8').read() or '').strip()
except Exception:
    pass
try:
    hptr_val = (open('/var/packages/hermes/var/workspace.home.path', 'r', encoding='utf-8').read() or '').strip()
except Exception:
    pass
out = {
    'configuredProviders': providers_payload,
    'workspaceDir': workspace or '/volume1/hermes',
    'configPath': cfg_path,
    'configExists': True,
...
if apply_now:
    try:
        ...
        state_dir = (os.path.dirname(cfg_path) if cfg_path else '/volume1/hermes/.hermes')
        user_dir = (os.path.dirname(state_dir) if state_dir.endswith('/.hermes') else state_dir)
        env['HOME'] = user_dir
        env['HERMES_CONFIG_PATH'] = cfg_path
        env['HERMES_STATE_DIR'] = state_dir
        env['HERMES_WORKSPACE_DIR'] = user_dir
        ...
        def stop_existing_gateway_for_restart():
            ...
            try:
                p = subprocess.run(
                    ['/var/packages/hermes/target/bin/hermes', 'gateway', 'stop', '--json'],
                    env=env,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
                    timeout=12,
                )
                attempts.append({'cmd': 'hermes gateway stop', 'rc': p.returncode, 'out': (p.stdout or b'').decode('utf-8', 'ignore')[-400:]})
            except Exception as e:
        ...
        try:
            subprocess.run(
                [
                    'bash', '-lc',
                    'source /var/packages/hermes/scripts/service-setup >/dev/null 2>&1; sync_dsm_package_info_port "' + str(gw_port) + '"'
                ],
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
                timeout=12
            ...
            try:
                info_file = '/var/packages/hermes/INFO'
...
if not base_url:
    ...
    raise SystemExit
headers = {'User-Agent': 'hermes-native-ui/1.0'}
if api_key:
...
out = {
  ...
  'weixin': ch.get('hermes-weixin') or ch.get('weixin') or {}
}
print(json.dumps(out, ensure_ascii=False))
PY
            exit 0
...
if isinstance(payload.get('qqbot'), dict):
    ...
    if has_cred:
        ...
        ch.pop('qqbot', None)
wx_payload = payload.get('hermes-weixin') if isinstance(payload.get('hermes-weixin'), dict) else payload.get('weixin')
if isinstance(wx_payload, dict):
    w = ch.setdefault('hermes-weixin', {})
    w['enabled'] = bool(wx_payload.get('enabled', True))
    # Auto-heal doctor requirement: channels.hermes-weixin.accounts.default
    acc = w.get('accounts')
...
stale_plugin_ids = {'feishu-hermes-plugin', 'hermes-qqbot', 'hermes-dingtalk', 'wecom-hermes-plugin'}
...
if not skip_reload:
    try:
        ...
        def is_gateway_running():
            ...
            try:
                pid_file = '/var/packages/hermes/var/hermes-gateway.runtime.pid'
                if os.path.exists(pid_file):
        ...
        if is_gateway_running():
            env['HERMES_GATEWAY_RESTART_START'] = '1'
            cmd = ['/var/packages/hermes/target/bin/hermes', 'gateway', 'restart', '--json']
...
out = {
  ...
  'weixin': channels_obj.get('hermes-weixin') or channels_obj.get('weixin') or {},
  'reloaded': reload_ok,
...
home_dir = sys.argv[1]
cfg_path = sys.argv[2]
debug_log = sys.argv[3] if len(sys.argv) > 3 else '/tmp/hermes-weixin-login.log'
worker_pid_file = sys.argv[4] if len(sys.argv) > 4 else '/tmp/hermes-weixin-login-worker.pid'
...
if not isinstance(allow, list):
    allow = []
if 'hermes-weixin' not in allow:
    allow.append('hermes-weixin')
plugins['allow'] = allow
entries = plugins.get('entries')
if not isinstance(entries, dict):
    entries = {}
entry = entries.get('hermes-weixin')
...
if url and qrcode:
    try:
        ...
        with open('/tmp/hermes-weixin-login-state.json', 'w', encoding='utf-8') as sf:
    ...
    print(json.dumps({'supported': True, 'qrUrl': url, 'sessionKey': 'hermes-weixin', 'roundId': round_id, 'message': '请用微信扫码完成登录', 'debugLog': debug_log}, ensure_ascii=False))
else:
...
cfg_path = sys.argv[1]
debug_log = sys.argv[2] if len(sys.argv) > 2 else '/tmp/hermes-weixin-login.log'
worker_pid_file = sys.argv[3] if len(sys.argv) > 3 else '/tmp/hermes-weixin-login-worker.pid'
...
def normalize_runtime_owned_state(path):
    ...
    try:
        import pwd, grp
        uid = pwd.getpwnam('sc-hermes').pw_uid
...
try:
    ...
    if s == 'confirmed' and st.get('bot_token') and st.get('ilink_bot_id'):
        ...
        try:
            ...
            state_dir = os.path.dirname(cfg_path) if cfg_path else '/volume1/hermes/.hermes'
            acc_dir = os.path.join(state_dir, 'hermes-weixin', 'accounts')
            ...
            try:
                ...
                if not isinstance(ents, dict):
                    ents = {}
                pe = ents.get('hermes-weixin')
                if not isinstance(pe, dict):
                    pe = {}
                pe['enabled'] = True
                ents['hermes-weixin'] = pe
                plugs['entries'] = ents
                ...
                log(f'weixin_hermes_json_update_err={e2}')

            try:
                normalize_runtime_owned_state(os.path.join(state_dir, 'hermes-weixin'))
...
env['XDG_CONFIG_HOME'] = state_dir + '/.config'
env['XDG_DATA_HOME'] = state_dir + '/.local/share'
cmd = ['/var/packages/hermes/target/bin/hermes', 'channels', 'logout', '--channel', 'hermes-weixin']
try:
    ...
    try:
        ...
        cfg = {}
    ch = cfg.setdefault('channels', {})
    for key in ('hermes-weixin', 'weixin'):
        if key in ch:
...
legacy_cmd = 'sudo -n /usr/syno/bin/synopkg restart hermes'
admin_fix_cmd = "sudo -n ln -sfn /var/packages/hermes/var/alias.hermes-terminal.conf /etc/nginx/conf.d/alias.hermes-terminal.conf && sudo -n sh -lc 'nginx -t && systemctl reload nginx'"
...
def run_admin_password_flow(user, password):
    ...
    if any(ch in user for ch in ' \t\r\n:'):
        ...
        return 126
    inner = "ln -sfn /var/packages/hermes/var/alias.hermes-terminal.conf /etc/nginx/conf.d/alias.hermes-terminal.conf && nginx -t && systemctl reload nginx"
...
alias_src = '/var/packages/hermes/var/alias.hermes-terminal.conf'
alias_dst = '/etc/nginx/conf.d/alias.hermes-terminal.conf'
...
def check_alias_https(tries=6, interval=0.4):
    for _ in range(tries):
        p = subprocess.run([
            ...
            '-H', 'Cookie: id=fake',
            'https://127.0.0.1:5001/hermes-terminal/token'
...
cfg_path = sys.argv[2] if len(sys.argv) > 2 else '/volume1/hermes/.hermes/hermes.json'
...
try:
    cli = '/var/packages/hermes/target/bin/hermes'
...
raw = sys.argv[1] if len(sys.argv) > 1 else '{}'
cfg = sys.argv[2] if len(sys.argv) > 2 else '/volume1/hermes/.hermes/hermes.json'
spawn_log = sys.argv[3] if len(sys.argv) > 3 else '/tmp/hermes-gateway.spawn.log'
gateway_pid_file = '/var/packages/hermes/var/hermes-gateway.runtime.pid'
try:
    payload = json.loads(raw or '{}')
except Exception:
    payload = {}
...
lock_path = '/var/packages/hermes/var/install_run.lock'
...
if action in ('start','restart'):
    try:
        if cfg and (not os.path.exists(cfg)):
            ...
            template = '/var/packages/hermes/target/app/hermes/config/hermes.template.json'
...
PY
        ...
        logs)
            printf 'Content-Type: application/json; charset=UTF-8\r\n\r\n'
            OCL_LOG="/tmp/hermes/hermes-$(date +%Y-%m-%d).log"
            [ -f "$OCL_LOG" ] || OCL_LOG="/tmp/hermes/hermes-$(date -d yesterday +%Y-%m-%d 2>/dev/null).log"
            [ -f "$OCL_LOG" ] || OCL_LOG="/tmp/hermes-0/hermes-$(date +%Y-%m-%d).log"
            [ -f "$OCL_LOG" ] || OCL_LOG="/tmp/hermes-0/hermes-$(date -d yesterday +%Y-%m-%d 2>/dev/null).log"
            APP_LOG="$LOG_FILE"
            SPAWN_LOG="${APP_VAR_DIR}/hermes-gateway.spawn.log"
            [ -f "$SPAWN_LOG" ] || SPAWN_LOG="/tmp/hermes-gateway.spawn.log"
...
<body>
  ...
  <script>
    const API_BASE = '/webman/3rdparty/hermes/index.cgi?native_api=1&action=';
    ...
    const BUILTIN_CHANNEL_PLUGINS = ['feishu','qqbot','dingtalk','wecom','hermes-weixin'];
    ...
    let terminalSuggest = ['hermes doctor', 'hermes gateway status', 'hermes gateway restart', 'hermes config validate'];
    let terminalGlobalKeyHooked = false;
    let terminalLocked = false;
    window.__hermesClientErrors = [];
    ...
    function captureClientError(type, payload) {
      try {
        ...
        window.__hermesClientErrors.push(rec);
        if (window.__hermesClientErrors.length > 50) window.__hermesClientErrors.shift();
    ...
    window.hermesClientErrors = function () {
      return (window.__hermesClientErrors || []).slice();
    };
      ...
      try {
        ...
        if (tab === 'status') {
          ...
          const rows = [
            ...
            ['用户文件夹路径', data.workspaceDir || '/volume1/hermes'],
        ...
        if (tab === 'models') {
          ...
          const workspaceDir = data.workspaceDir || '/volume1/hermes';
        ...
        if (tab === 'channels') {
          ...
          content.innerHTML = ''
            ...
            + '      <option value="hermes-weixin">微信</option>'
            + '    </select></div>'
    ...
    function setInstallButtonsBusy(actionName, busy) {
      ...
      if (busy) {
        if (actionName === 'stop') {
          ...
          startBtn.textContent = '启动 Hermes Agent';
          stopBtn.textContent = '停止中...';
          return;
        }
        startBtn.disabled = true;
        stopBtn.disabled = false;
        startBtn.textContent = actionName === 'start' ? '启动中...' : '启动 Hermes Agent';
        stopBtn.textContent = '强制停止 Hermes Agent';
        return;
      }
      ...
      stopBtn.disabled = false;
      startBtn.textContent = '启动 Hermes Agent';
      stopBtn.textContent = running ? '停止 Hermes Agent' : '强制停止 Hermes Agent';
    }
    ...
    function switchChannelDialog() {
      ...
      if (t === 'feishu') {
        ...
        window.__weixinHadChannelBefore = !!((data.configuredChannelIds || []).includes('hermes-weixin') || (data.configuredChannelIds || []).includes('weixin'));
    ...
    async function saveChannelDialog(opts) {
      ...
      if (t === 'feishu') {
        ...
        payload = { 'hermes-weixin': { enabled: true } };
      }
      const btn = document.getElementById('btn_channel_save');
    ...
    function isWeixinDialogActive() {
      ...
      return !!(m && m.style.display !== 'none' && sel && sel.value === 'hermes-weixin');
    }
    ...
    function isDsmPanelContext() {
      try {
        ...
        return p.indexOf('/webman/3rdparty/hermes/') === 0 || p.indexOf('/webman/index.cgi') === 0;
      } catch (_) {
        return false;
      }
    }
