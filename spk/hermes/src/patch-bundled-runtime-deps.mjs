...
if (!root) {
  console.error('[patch-bundled-runtime-deps] usage: node patch-bundled-runtime-deps.mjs <hermes-bundle-dir>');
  process.exit(2);
}
...
for (const file of doctorCandidates) {
  ...
  const newMissingLine = `const packagedAppRoot = path.join(params.packageRoot, "app", "hermes");
