...
if (!root) {
  console.error('[patch-bundled-channel-entries] usage: node patch-wecom-bundled-entry.mjs <hermes-bundle-dir>');
  process.exit(2);
}
...
const patchPackageHermes AgentMeta = (file, nextPluginId, nextChannelId, nextExtensions) => {
  if (!fs.existsSync(file)) return false;
  const json = JSON.parse(fs.readFileSync(file, 'utf8'));
  let changed = false;
  json.hermes = json.hermes && typeof json.hermes === 'object' ? json.hermes : {};
  if (nextPluginId) {
    if (json.hermes.id !== nextPluginId) {
      json.hermes.id = nextPluginId;
      changed = true;
    }
  }
  json.hermes.channel = json.hermes.channel && typeof json.hermes.channel === 'object' ? json.hermes.channel : {};
  if (nextChannelId && json.hermes.channel.id !== nextChannelId) {
    json.hermes.channel.id = nextChannelId;
    changed = true;
  }
  if (Array.isArray(nextExtensions) && nextExtensions.length > 0) {
    const cur = Array.isArray(json.hermes.extensions) ? json.hermes.extensions : [];
    if (JSON.stringify(cur) !== JSON.stringify(nextExtensions)) {
      json.hermes.extensions = nextExtensions;
      changed = true;
    }
  }
...
const patchFeishu = (dir) => {
  ...
  // Keep official hermes-lark entry contract unchanged.
  return 0;
};
...
`,
  ...
  patchPluginManifestContract(path.join(dir, 'hermes.plugin.json'), {
    id: 'qqbot',
    channels: ['qqbot'],
    extensions: ['./index.ts']
  });
  patchPackageHermes AgentMeta(path.join(dir, 'package.json'), 'qqbot', 'qqbot', ['./index.ts']);
  return 1;
};
...
`,
  ...
  patchPackageHermes AgentMeta(path.join(dir, 'package.json'), 'dingtalk', 'dingtalk', ['./index.ts']);
  return 1;
};
...
`,
  ...
  patchPackageHermes AgentMeta(path.join(dir, 'package.json'), 'wecom', 'wecom', ['./index.js']);
  return 1;
};
...
`,
  ...
  patchPluginManifestContract(path.join(dir, 'hermes.plugin.json'), {
    id: 'hermes-weixin',
    channels: ['hermes-weixin'],
    extensions: ['./index.ts']
  });
  patchPackageHermes AgentMeta(path.join(dir, 'package.json'), 'hermes-weixin', 'hermes-weixin', ['./index.ts']);
  return 1;
};
...
const targets = [
  ...
  { name: 'node-feishu', dir: path.join(root, 'node_modules', '@larksuite', 'hermes-lark'), patch: patchFeishu },
  { name: 'node-qqbot', dir: path.join(root, 'node_modules', '@tencent-connect', 'hermes-qqbot'), patch: patchQQBot },
  ...
  { name: 'node-weixin', dir: path.join(root, 'node_modules', '@tencent-weixin', 'hermes-weixin'), patch: patchWeixin },
  ...
  { name: 'hermes-weixin', dir: path.join(root, 'dist', 'extensions', 'hermes-weixin'), patch: patchWeixin },
];
